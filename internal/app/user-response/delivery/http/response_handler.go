...
func NewResponseHandler(m *mux.Router, rs user_response.Usecase, sanitizer *bluemonday.Policy, logger *zap.SugaredLogger, sessionStore sessions.Store) {
	...
	// TODO: nozim: Почему для "/jobs/{id:[0-9]+}/response}" метод options возращает 404, а для "/jobs/response/{id:[0-9]+}" 200?
	m.HandleFunc("/jobs/response/{id:[0-9]+}", handler.HandleResponseJob).Methods(http.MethodPost, http.MethodOptions)
